...
exports.resolveMainPlace = resolveMainPlace;
exports.getReplacementCandidates = getReplacementCandidates;
exports.replaceCourseStop = replaceCourseStop;
exports.generateCourses = generateCourses;
...
function getBudgetLimit(request) {
...
function getPreferenceScore(place, preferences) {
    return Math.round((place.scores.romantic * (preferences.lovely / 5) +
...
function candidateScore(place, preferences, weather, detailedOptions) {
    return (0, scoring_1.calculatePlaceScore)(place, preferences, weather, detailedOptions).total;
}
function computeCourseScore(stops, preferences, weather, detailedOptions) {
...
function exceedsBudgetByMoreThan20Percent(totalCost, request) {
    const allowed = getBudgetLimit(request) * 1.2;
    return totalCost > allowed;
}
function isCourseWithinBudgetAndTime(course, request) {
...
function buildReasons(course, request, fixedPlace) {
    const reasons = [];
...
function evaluateReplacementCandidate(candidate, previousStop, nextStop, request, weather, detailedOptions, transportationMode, currentCourse, replacementIndex) {
    const preferenceScore = getPreferenceScore(candidate, request.preferences);
    const previousTravel = (0, distance_1.estimateTravelMinutes)(previousStop.lat, previousStop.lng, candidate.lat, candidate.lng, transportationMode);
    const nextTravel = (0, distance_1.estimateTravelMinutes)(candidate.lat, candidate.lng, nextStop.lat, nextStop.lng, transportationMode);
...
function buildCourseMetrics(course, request, weather, detailedOptions, transportationMode, fixedPlace) {
    const recalculatedStops = [];
    ...
    course.stops.forEach((stop, index) => {
        ...
        const travelMinutes = (0, distance_1.estimateTravelMinutes)(previousPlace.lat, previousPlace.lng, stop.place.lat, stop.place.lng, transportationMode);
    ...
    const totalDistanceKm = recalculatedStops.reduce((sum, stop, index) => {
        ...
        return sum + (0, distance_1.calculateDistanceKm)(recalculatedStops[index - 1].place.lat, recalculatedStops[index - 1].place.lng, stop.place.lat, stop.place.lng);
    }, 0);
    const nextCourse = {
        ...course,
        ...
        averagePreferenceScore: Math.round(recalculatedStops.reduce((sum, stop) => sum + getPreferenceScore(stop.place, request.preferences), 0) / recalculatedStops.length),
...
function resolveMainPlace(request, weather, detailedOptions, selectedMainPlace, mainPlaceId) {
    if (selectedMainPlace) {
        return selectedMainPlace;
    }
    const normalizedMainPlaceId = mainPlaceId?.trim();
    if (normalizedMainPlaceId) {
        const matchedPlace = places_1.mockPlaces.find((place) => place.id === normalizedMainPlaceId);
        if (matchedPlace) {
            return matchedPlace;
        }
    }
    const fallbackActivities = places_1.mockPlaces
    ...
    return fallbackActivities[0] ?? places_1.mockPlaces[0];
}
function getReplacementCandidates(course, request, weather, detailedOptions, index, selectedMainPlace, mainPlaceId) {
    ...
    const transportationMode = request.transportation_mode === "PUBLIC" ? "PUBLIC" : request.transportation_mode === "CAR" ? "CAR" : "WALK";
    const candidates = places_1.mockPlaces
        .filter((place) => place.category === currentCategory)
        .filter((place) => !usedIds.has(place.id))
        .filter((place) => place.id !== fixedPlace.id)
        .filter((place) => {
        const arrivalMinutes = parseTimeToMinutes(previousStop.departureTime) + (0, distance_1.estimateTravelMinutes)(previousStop.place.lat, previousStop.place.lng, place.lat, place.lng, transportationMode);
        const arrivalDate = buildDateTime(request.date, formatTime(arrivalMinutes));
        return (0, opening_hours_1.isPlaceOpenAt)(place.opening_hours, arrivalDate);
    })
        .filter((place) => {
        const arrivalMinutes = parseTimeToMinutes(previousStop.departureTime) + (0, distance_1.estimateTravelMinutes)(previousStop.place.lat, previousStop.place.lng, place.lat, place.lng, transportationMode);
        const departureMinutes = arrivalMinutes + place.avg_stay_duration_min;
        const nextArrivalMinutes = departureMinutes + (0, distance_1.estimateTravelMinutes)(place.lat, place.lng, nextStop.place.lat, nextStop.place.lng, transportationMode);
        ...
        .map((place) => ({
        place,
        score: evaluateReplacementCandidate(place, previousStop.place, nextStop.place, request, weather, detailedOptions, transportationMode, course, index),
    }))
        .sort((left, right) => right.score - left.score)
        .slice(0, 3);
    return candidates;
}
function replaceCourseStop(course, request, weather, detailedOptions, index, replacementPlace, selectedMainPlace, mainPlaceId) {
    const fixedPlace = resolveMainPlace(request, weather, detailedOptions, selectedMainPlace, mainPlaceId);
    if (index <= 0 || index >= course.stops.length) {
        return null;
    }
...
function generateCourses(options) {
    ...
    const transportationMode = request.transportation_mode === "PUBLIC" ? "PUBLIC" : request.transportation_mode === "CAR" ? "CAR" : "WALK";
    const fixedPlace = resolveMainPlace(request, weather, detailedOptions, selectedMainPlace, mainPlaceId);
    const restaurants = places_1.mockPlaces.filter((place) => place.category === "RESTAURANT");
    const activities = places_1.mockPlaces.filter((place) => place.category === "ACTIVITY");
    const cafes = places_1.mockPlaces.filter((place) => place.category === "CAFE");
    const photoSpots = places_1.mockPlaces.filter((place) => place.category === "PHOTO_SPOT");
    const baseStops = [
    ...
    const candidates = [];
    const rejected = [];
    const buildCourse = (type, categoryOrder, seed) => {
        const fullStops = [];
        ...
        const addStop = (place, travelMinutes) => {
            ...
            totalDistanceKm += (0, distance_1.calculateDistanceKm)(currentLat, currentLng, place.lat, place.lng);
        ...
        if (!(0, opening_hours_1.isPlaceOpenAt)(initialStop.opening_hours, initialArrivalDate)) {
            return null;
        }
        ...
        for (const category of categoryOrder) {
            ...
            const travelMinutes = (0, distance_1.estimateTravelMinutes)(currentLat, currentLng, chosen.lat, chosen.lng, transportationMode);
        ...
        const course = {
            ...
            totalTravelMinutes,
            stops: fullStops,
            averagePreferenceScore: Math.round(fullStops.reduce((sum, stop) => sum + getPreferenceScore(stop.place, request.preferences), 0) / fullStops.length),
